 ...
 * It never widens what the addon accepts — an address this file admits still
...
function acceptedHrps(profile: SwarmNetworkProfile): readonly string[] {
  const extra = profile.id === SwarmProfileIdEnum.testnet ? TESTNET_EXTRA_HRPS : MAINNET_EXTRA_HRPS;
...
export function checkAddressForProfile(address: string, profile: SwarmNetworkProfile): AddressVerdict {
  ...
  const other =
    profile.id === SwarmProfileIdEnum.mainnet ? SWARM_TESTNET_PROFILE : SWARM_MAINNET_PROFILE;
  const otherHome = swarmHomeOf(other);
  const otherExtra = other.id === SwarmProfileIdEnum.testnet ? TESTNET_EXTRA_HRPS : MAINNET_EXTRA_HRPS;
