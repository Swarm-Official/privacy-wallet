...
The `Privacy wallet Windows test build` GitHub Actions workflow builds a portable, unsigned Windows x64 ZIP. Its draft integration SDK is reconstructed from the exact upstream revision and patch digest in `patches/privacy-sdk-v2.json`; full SDK feature and benchmark release gates remain separate and incomplete. The artifact includes that manifest, the wallet source commit, dependency lockfile and ZIP checksum. It does not publish a release or sign the executable. The unsigned development artifact is distinct from the signed official upstream wallet used for the earlier Regtest compatibility exercise.
